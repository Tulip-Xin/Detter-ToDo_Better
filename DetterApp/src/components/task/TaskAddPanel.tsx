/**
 * TaskAddPanel 组件
 * 任务添加面板UI，包含日期选择、标题输入、描述输入、优先级选择等
 */
...
const TaskAddPanel: React.FC<TaskAddPanelProps> = ({
  onSave,
  onCancel,
  initialPriority,
  initialDate,
  ...
  const [priority, setPriority] = useState<Priority>(initialPriority || 'important');
  ...
  const handleSave = async () => {
    ...
    // 计算当前优先级的最大order值
    const tasksWithSamePriority = existingTasks.filter(
      (t) => t.priority === priority && !t.completed && !t.archived
    );
    const maxOrder = tasksWithSamePriority.length > 0
      ? Math.max(...tasksWithSamePriority.map((t) => t.order))
      : -1;
